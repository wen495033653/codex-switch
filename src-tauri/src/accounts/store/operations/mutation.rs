use super::super::{
    model::{account_id_from_account, normalize_account, sort_accounts_by_last_used},
...
pub(crate) fn add_account_to_store(account: Value, mark_active: bool) -> Result<Value, String> {
    ...
    if let Some(index) = accounts
        .iter()
        .position(|existing| account_id_from_account(existing).unwrap_or_default() == account_id)
    {
...
pub(crate) fn mark_store_account_used(account_id: &str) -> Result<Value, String> {
    let mut store = read_store_value()?;
    let accounts = store
        .get_mut("accounts")
        .and_then(Value::as_array_mut)
    ...
    sort_accounts_by_last_used(accounts);
    store["active_id"] = Value::String(account_id.to_string());
    write_store_value(&store)?;
    Ok(store)
}

pub(crate) fn remove_store_account(account_id: &str) -> Result<Value, String> {
    let mut store = read_store_value()?;
    let accounts = store
        .get_mut("accounts")
        .and_then(Value::as_array_mut)
    ...
    accounts.retain(|account| account_id_from_account(account).unwrap_or_default() != account_id);
