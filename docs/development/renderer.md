...
检查对比度时，在替身页面里对每段可见文字计算它和实际背景的对比度（逐层合成祖先的半透明背景），列出低于 3:1 的元素，浅色、深色主题各跑一遍所有页面。

## 可读性修正（2026-09-26）

用上面的替身在 1024×900（主窗口默认宽度）下，把账号、API、Codex、会话、设置五个页面在浅色、深色主题下都渲染了一遍，按测量结果修改：

- **提示语言。** 没有额度数据、正在同步、状态异常时的提示，前端默认文字和后端写入的"正在同步"状态都是英文，中文界面也显示英文（例如 `Usage data missing, please refresh`）。现在改成中文原文，英文界面通过翻译表显示原来的英文。
- **对比度。** 共同原因是强调色 `#339cff`：作为文字放在浅蓝底上只有 2.5:1，作为底色配白字只有 2.9:1。在 `visual-refresh.css` 新增两个变量：
  - `--accent-text`（浅色 `#0a6ccf`，深色 `#4aa8ff`），用于浅色底上的强调色文字；
  - `--accent-fill`（`#1a6fd1`），用于白字下的填充底。

  具体修改如下：

  | 位置 | 修改前 | 修改后 |
  | --- | --- | --- |
  | Codex 页"重启 Codex"（浅色） | 1.31:1，只写了深色主题配色，看起来像禁用 | 改用变量 |
  | 当前 PID（浅色） | 2.38:1 | 改用变量 |
  | "当前"徽标（浅色） | 2.53:1 | 改用变量 |
  | 分页当前页、深色主题主按钮 | 2.86:1 | 改用 `--accent-fill` |
  | 浅色额度绿色 | `#00a240` | `--success` 改为 `#00853a` |
  | 深色筛选数量 | 2.25:1，透明度 0.5 | 透明度改为 0.72 |
  | 深色空状态文字 | 2.06:1，写死 `#52525b` | 改用 `--text-tertiary` |

- **选中与悬停。** 顶部导航、账号筛选、设置分页的"当前"样式和"悬停"样式几乎相同：深色主题两者都是 `#343434`，浅色主题一个是 5% 灰、一个是 `#f3f3f3`。鼠标悬停在别的项上时，看不出当前在哪一项。现在"当前"统一用已有的 `--surface-selected` 配 `--accent-text`，"悬停"仍是灰色。
- **账号卡片被裁。** 卡片高度由网格固定。徽标多（例如 `PLUS`、`登录已失效`、到期日、可重置次数）时换成两行，标题区从 52px 变成 78px，正文里的 token 汇总被压到最小高度，"Token"标题被裁掉。现在卡片显示提示（出错、无额度数据）时，由提示框吸收压缩，token 汇总保持完整。显示额度条的卡片不变。
- **会话列表时间列。** 以前显示完整本地时间（例如 `2026/9/26 21:31:…`），在约 100px 的列里被截断。现在当年的会话显示 `09/26 21:31`，往年的显示年月日，完整时间放在悬停提示里。其他位置（右键菜单等）仍显示完整时间。

验证：
- **对比度巡检（替身后端）。** 修改前，浅色主题有 7 处低于 3:1；深色主题有 11 处，其中筛选数量 4 处，主按钮的文字和图标 5 处，分页当前页和会话空状态各 1 处。修改后深色为 0；浅色只剩 Codex 页"仅 API 模式"（2.86:1），它所在的整行因为不可用被整体调暗，这次没有改。
- **截图核对。** 出错账号卡片的 Token 区完整显示；中文界面提示为"暂无额度数据，请刷新"，英文界面为 `Usage data missing, please refresh`；会话时间中文为 `09/26 21:42`、英文为 `09/26, 21:42`，都没有截断，悬停提示是完整时间。
- **离线检查。** `npm run check`（卫生检查、前端测试、构建）通过；Rust `cargo test` 337 个通过，fmt、clippy 通过。
- **900px 最小宽度。** 只检查了账号页（900×700）：没有横向溢出，6 张卡片的 token 汇总都没有被裁，对比度巡检 0 处。
- **没有验证的。** 没有在安装版里看过实际窗口；各类弹窗和 token 详情抽屉没有逐一截图。

## 设置页的长路径（2026-09-26）

- 数据目录路径很长时（例如开发预览的沙盒目录）会把设置页撑出窗口，"打开"按钮被挤到窗口外。路径文字本身设了省略号和 `min-width: 0`，但外层 `settings-page-content`、`settings-section`、`settings-path-card` 都是网格子项，默认按内容的最小宽度计算列宽，列宽被撑到路径全长。在 940px 宽的面板里，列宽被撑到 1355px。
- 现在这三层都加了 `min-width: 0`，路径按面板宽度截断，悬停提示仍是完整路径。
- 开发版导航栏下的琥珀色横条（`.app.dev-build .navbar`）按用户要求删除，"DEV BUILD"水印保留。
- 验证（替身后端，1024×900 深色主题，长路径）：修改后路径宽 788px 并出现省略号，面板 940px，页面没有横向滚动，"打开"按钮在窗口内，导航栏下边框为普通分隔线。开发预览窗口通过热更新加载了修改。
...
- API 配置弹窗里的“配置预检”区块删除。`useApiModeDraft` 从来只把 `precheck` 设为 `null`，这个区块、提交按钮的“预检中...”文案、对应的 CSS 规则（`styles/api.css`、`styles/theme/api.css` 的 `.api-profile-modal-precheck*`，以及 `styles/visual-refresh.css` 选择器列表里的这一项）和两个英文词条（`配置预检`、`预检中...`）都不会生效。验证：`git grep` 确认前端和后端都不再引用这些类名和词条；渲染检查（替身后端）中英文下新增、编辑弹窗的 DOM 与改动前逐字相同，保存流程相同，预检弹窗里仍使用那条共用规则的元素计算样式不变。
- 删除没有引用的导出：`utils/apiPrecheck.js` 的 `hasFreshSuccessfulApiPrecheck`、`mergeApiTestResult`、`getApiPrecheckFailureMessage`（以及只为它的输出准备的翻译规则 `/^API 预检失败：/`），`utils/appState.js` 的 `buildApiModePayload`、`getApiProviderDisplayName`，`utils/auth.js` 转出的 `parseJwt`（`auth/jwt.js` 内部仍在用）。删除没有来源的英文词条 `Codex 代理已关闭`、`Codex 代理已启用`：前端没有用到，后端返回的是 `Codex 代理配置已启用/已关闭，重启 Codex 后生效。`，那两条词条保留。验证：`git grep` 在 `renderer/`、`scripts/`、`src-tauri/src` 中都找不到这些名字和原文；`npm run check` 通过。
- token 统计的 `getWindow`、`hasUsage`、`formatTokens`、`formatCost` 在卡片摘要和详情抽屉里各有一份相同的实现，合并到 `utils/usageStatsFormat.js`。验证：`scripts/test-usage-stats-format.mjs`；用虚构统计数据对两个组件做静态渲染（中英文 × 空、小额、大额三组），改动前后 12 份标记逐字相同。
