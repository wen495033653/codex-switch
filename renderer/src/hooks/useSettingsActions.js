...
export function useSettingsActions({
  ...
  const showProxySaveFeedback = (result) => {
    ...
    if (feedback.restartRequired) {
      setCodexRestartNoticeMessage('Codex 代理配置已保存，重启 Codex 后生效。');
  ...
  const setCodexModelInstructionsEnabled = async (enabled, overwriteLocal = null) => {
    ...
    try {
      ...
      setSettingsDraft(settings);
      toast(getErrorMessage(err, enabled ? '启用 gpt破限失败' : '关闭 gpt破限失败'), 7000);
    } finally {
      setSavingCodexModelInstructions(false);
    }
  };
